...
@foreach($cards as $index=>$card)
    <div class="bundle-box @if($card->sellable) sellable @endif">
        <h4>{{$card->title}}</h4>
        ...
        <span class="price">Price: <b>$</b><input wire:model="cards.{{$index}}.price" type="number" step="0.01" value="{{$card->price}}" @if($card->sellable) disabled @endif> </span> <br>
        @if($card->sellable)
